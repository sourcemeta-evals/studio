...
function App() {
  ...
  useEffect(() => {
    const savedState = (vscode as any).vsCodeApi.getState() as { activeTab?: TabType } | undefined;
  ...
  useEffect(() => {
    if (state?.blockedByMetaschema) {
      ...
      (vscode as any).vsCodeApi.setState({ activeTab: 'metaschema' });
    }
  }, [state?.blockedByMetaschema]);

  const handleTabChange = (tab: TabType) => {
    setActiveTab(tab);
    (vscode as any).vsCodeApi.setState({ activeTab: tab });
  };
