...
export function Footer({ cliVersion, extensionVersion }: FooterProps) {
  const handleStarRepo = () => {
    (vscode as any).postMessage({ command: 'openExternal', url: 'https://github.com/sourcemeta/studio' });
  };

  const handleReportBug = () => {
    (vscode as any).postMessage({ command: 'openExternal', url: 'https://github.com/sourcemeta/studio/issues/new' });
  };

  const handleSourcemetaClick = () => {
    (vscode as any).postMessage({ command: 'openExternal', url: 'https://www.sourcemeta.com/' });
  };
